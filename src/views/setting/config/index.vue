  ...
  const getConfigData = async (id) => {
    ...
    optionsArray.value[id] = response.data.map(item => {
      let option = {
        ...
        placeholder: item.key == 'index_banner' ? t('adm.setting:index_banner_placeholder') : item.key == 'site_url' ? t('adm.setting:site_url_placeholder') : item.key == 'websocket_url' ? t('adm.setting:websocket_url_placeholder') : item.name,
        dataIndex: item.key, formType: item.input_type,
        dict: {}, labelWidth: '120px', extra: '', tooltip: item.remark ? t('adm.setting:' + item.remark) : '',
      }
